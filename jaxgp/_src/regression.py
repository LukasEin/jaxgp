...
class BaseGPR:
    ...
    def train(self, X_data: Array, Y_data: Array) -> None:
        ...
        self.fit_matrix, self.fit_vector = self.forward(self.kernel_params, self.noise, Y_data, *self.further_args)

    def forward(self, kernel_params: Array, noise, Y_data: Array, *further_args) -> Tuple[Array, Array]:
    ...
    def eval(self, X: Array, return_std=False) -> Union[Tuple[Array, Array], Array]:
        ...
        if return_std:
            return self._meanstd(self.kernel_params, self.noise, X, self.fit_matrix, self.fit_vector, *self.further_args)
        
        return self._mean(self.kernel_params, self.noise, X, self.fit_matrix, self.fit_vector, *self.further_args)
...
class ExactGPR(BaseGPR):
    ...
    def forward(self, kernel_params: Array, noise, Y_data: Array, X_split: list[Array]) -> Tuple[Array, Array]:
        ...
        K_NN = self._CovMatrix_Kernel(X_split[0], X_split[0], params=kernel_params)
        for i,elem in enumerate(X_split[1:]):
            K_mix = self._CovMatrix_Grad(X_split[0], elem, index=i, params=kernel_params)
            K_NN = jnp.concatenate((K_NN,K_mix),axis=1)
        
        for i,elem_1 in enumerate(X_split[1:]):
            K_mix = self._CovMatrix_Grad(X_split[0], elem_1, index=i, params=kernel_params)

            for j,elem_2 in enumerate(X_split[1:]):
                K_derivs = self._CovMatrix_Hess(elem_1, elem_2, index_1=i, index_2=j, params=kernel_params)
        ...
        return (jnp.eye(len(K_NN)) * (noise**2 + 1e-6) + K_NN), (Y_data)
    
    @partial(jit, static_argnums=(0,))
    def _mean(self, kernel_params: Array, noise, X: Array, fitmatrix: Array, fitvector: Array, X_split: Array) -> Array:
        '''
            calculates the posterior mean for all points in X
        '''
        full_vectors = self._CovMatrix_Kernel(X, X_split[0], params=kernel_params)
        for i,elem in enumerate(X_split[1:]):
            deriv_vectors = self._CovMatrix_Grad(X, elem, index=i, params=kernel_params)
    ...
    @partial(jit, static_argnums=(0,))
    def _meanstd(self, kernel_params: Array, noise, X: Array, fitmatrix: Array, fitvector: Array, X_split: Array) -> Tuple[Array, Array]:
        '''
        ...
        X_cov = self._CovVector_Id(X, kernel_params)  
    ...
    def _kernelNegativeLogLikelyhood(self, kernel_params: Array, noise: Union[Array, float], Y_data: Array, X_split: list[Array]) -> float:
        ...
        '''
        # calculates the full covariance matrix
        fit_matrix, fit_vector = self.forward(kernel_params, noise, Y_data, X_split)
        fit_vector = fit_vector.reshape(-1)
...
class SparseGPR(BaseGPR):
    ...
    @partial(jit, static_argnums=(0,))
    def forward(self, kernel_params: Array, noise, Y_data: Array, X_ref: Array, X_split: list[Array]) -> Tuple[Array, Array]:
    ...
    @partial(jit, static_argnums=(0,))
    def _mean(self, kernel_params: Array, noise, X, fitmatrix: Array, fitvector: Array, X_ref: Array, X_split: list[Array]) -> Array:
        '''
            calculates the posterior mean for all points in X
        '''
        # calculates the caovariance between the test points and the reference points
        ref_vectors = self._CovMatrix_Kernel(X, X_ref, kernel_params)
        
        means = ref_vectors@solve(fitmatrix,fitvector)#,assume_a="pos")
        return means
    
    @partial(jit, static_argnums=(0,))
    def _meanstd(self, kernel_params: Array, noise, X: Array, fitmatrix: Array, fitvector: Array, X_ref: Array, X_split: list[Array]) -> Tuple[Array, Array]:
        '''
            calculates the posterior mean and std for all points in X
        '''
        # calculates the caovariance between the test points and the reference points
        ...
        second_temp = noise**2 * self._build_xTAx(fitmatrix, ref_vectors)
