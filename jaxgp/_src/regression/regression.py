...
class ExactGPR:
    ...
    def train(self, X_data: Tuple[ndarray, ndarray], Y_data: ndarray) -> None:
        ...
        solver = ScipyBoundedMinimize(fun=jit(likelyhood.full_kernelNegativeLogLikelyhood), method=self.optimize_method, callback=self.logger)
...
class GPRnograd:
    ...
    def train(self, X_data: ndarray, Y_data: ndarray) -> None:
        ...
        if self.is_sparse:
            solver = ScipyBoundedMinimize(fun=jit(likelyhood.sparse_kernelNegativeLogLikelyhood_nograd), method=self.optimize_method, callback=self.logger)
            result = solver.run(self.kernel_params, (1e-3,jnp.inf), self.X_data, Y_data, self.X_ref, self.noise, self.kernel)
        else:
            solver = ScipyBoundedMinimize(fun=jit(likelyhood.full_kernelNegativeLogLikelyhood_nograd), method=self.optimize_method, callback=self.logger)
